...
Stanza-specific mortality at low stock size $M_{0,s}$, is defined as:
...
\begin{equation}
  \label{eq:11a}
  M_{0,s} = \rho_s + \tau_s \right [ R_0 \sum_{a = cann_a} (lx_a spn_a) \left ]
\end{equation}

Here, $\rho$ refers to mortality rates when cannibalism does not occur, $\tau_s$ is the cannibalism-dependent parameter, and $R_0$

<p style="color:red">Alternatively, this can be formulated as:</p>
...
and $C$ is the sum of animals in age classes where cannibalism may occur.

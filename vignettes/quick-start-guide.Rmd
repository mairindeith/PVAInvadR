...
Now you can use any .csv editing software to define your PVA parameters.

Note: for any parameters that differ between species stanzas or control gear types, you must define all parameters for all stanzas or gears.
Separate multiple values with a semi-colon, `;`, in the .csv file.

For example,
For any multiple-value parameters, values should be provided in order from smallest  (for example, biological parameters which take on different values depending on cohort, or control parameters which differ based on different gear types applied).
...
```{r loadparams}
inputs <- load_pva_parameters("path_to_filled_parameter_file.csv")
```

## Running a PVA

For the rest of the quick start guide, we're going to use `PVAInvadR`'s built-in dataset.

```{r dataload}
data("fusee")
inputs <- fusee
```
Using these input parameters, let's run a PVA

```{r loadparams}
pva_results <- PVA(inputs)
```
...
These include estimates of cost, population size,

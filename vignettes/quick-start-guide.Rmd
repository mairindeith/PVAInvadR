...
```{r loadparams}
inputs <- load_pva_parameters("PATH/TO/FILLED_TEMPLATE.csv")
```
...
Load the example dataset and save these parameters with the variable name `inputs`.
...
Let's also include a plot of simulated abundance over time by setting `create_plot = TRUE`.

```{r firstpva}
pva_results <- PVA(inputs, create_plot = TRUE)
```
...
_ScenarioB_ will also increase the effort as above, and will also sample gear 1 at an earlier timestep in the year (parameter `t_start_A_1`) from 15 to 10.
...
```{r decsetup_csv}
decision_setup(inputs, scen_names = scenarios, csv = TRUE, csv_path = "PATH/TO/DECISION_TEMPLATE.csv")
```
...
![presetup](img/dec_setup_pre.png)

Modified values:

![presetup](img/dec_setup_post.png)
...
Now, all you need to do is run the `decision` function and .
`decision` requires the original parameter inputs, `inputs`, and the path to the scenario csv you just filled in (`decision_csv`, replace `PATH/TO/DECISION_TEMPLATE.csv` below to the file with your scenario-specific control parameters).
...
decision(inputs,
  ...
  parallel = TRUE,
  pretty = TRUE) # Turn off for a manipulatable data frame format
```
...
`rank_uncertainty` requires only the base parameter inputs, `inputs`, the .csv of control parameters modified by scenario, and what percent the biological parameters should be varied (but the default value is 15%, resulting in 85% and 115% the original value being evaluated).
